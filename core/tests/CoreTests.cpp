...
void TestBitrateBackoff() {
    std::printf("[M1] Bitrate: back off on loss, clamp at floor...\n");
    ...
    // Mất gói liên tục phải dừng ở sàn chứ không tụt về 0.
    uint64_t now = 3'000'000;
    for (int i = 0; i < 40; ++i, now += 1'000'000) {
        d = c.Update(Fb(9), now);
        if (d.changeBitrate) c.CommitBitrate(d.bitrateBps);
    }
    // Dừng SÁT sàn chứ không đúng bằng sàn: bước cuối cùng xuống 1 Mbps chỉ còn
    // ~13 kbps, nhỏ hơn ngưỡng chống-dao-động 2% nên bị bỏ qua. Chấp nhận được —
    // sàn vốn là ước lượng "dưới mức này hình nát", lệch 1.4% không đổi gì — nhưng
    // điều PHẢI đúng là không bao giờ tụt xuống dưới nó.
    ...
    // Và đã chạm sàn thì đứng yên, không gõ cửa encoder mỗi giây nữa.
...
// Encoder từ chối đổi bitrate -> controller KHÔNG được coi như đã đổi, nếu không
// lần Feedback sau sẽ tính tiếp từ một mức chưa bao giờ có thật.
void TestBitrateUncommitted() {
    std::printf("[M1] Bitrate: rejected change doesn't move the controller...\n");
    BitrateController c(20'000'000, 1'000'000);
...
void TestFecHysteresis() {
    std::printf("[M1] FEC: on immediately, off only after 5 clean seconds...\n");
    BitrateController c(20'000'000, 1'000'000);
    ...
    // Mất gói lại giữa chuỗi sạch phải reset bộ đếm, không cộng dồn.
...
void TestLinkStatsWindow() {
    std::printf("[M1] LinkStats: per-window deltas and rates...\n");
    LinkStats ls(0);
...
// Cửa sổ hiếm khi đúng 1.000s (vòng lặp client bị recvfrom chặn tới 100ms); chia
// theo hằng số 1s thay vì thời gian thật sẽ thổi phồng fps/kbps.
void TestLinkStatsUsesRealElapsed() {
    std::printf("[M1] LinkStats: rates use the real window length...\n");
    LinkStats ls(0);
    Reassembler::Stats st{};
...
void TestFeedbackFromWindow() {
    ...
    // Đường sạch vẫn phải gửi Feedback được: host cần tín hiệu "thông" mới dám nới.
    LinkWindow clean;
...
int main() {
    ...
    TestInputReorder();
    std::printf("=== GD5 M1: self-test congestion control + link stats (offline) ===\n");
    TestBitrateBackoff();
    TestBitrateRecovery();
