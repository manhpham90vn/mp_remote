...
- ✅ **Ba fix từ log chẩn đoán 2026-07-21** (host QSV → client Intel, LAN sạch —
  xem `09-diagnostics.md`): (1) **VBV ~2 frame cho MfEncoder** — QSV từng đẻ IDR
  196 KB (~5× ngân sách frame), thủ phạm chùm mất gói Wi-Fi ở `06 §7b`;
  (2) **keepalive ~2fps khi nguồn tĩnh** — MFT async ngậm frame cuối tới khi có input
  mới (đo e2e ảo ~30 s, nội dung trễ một nhịp), bơm lại frame cache để đẩy ra;
  (3) **dựng decoder trên thread Decode** thay vì thread Recv (MfDecoder init ~150 ms
  từng chặn recvfrom đúng lúc IDR mở màn dồn về — `recv_stall busy_ms=148`).
  CHỜ đo lại trên cặp máy thật để xác nhận cỡ IDR mới và e2e hết ảo.
